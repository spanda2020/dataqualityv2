...
if df_app_config.count() == 0:
    print(" ************* DQ_APP_CONFIG has no matching record for . Please check the input Parameter or DQ_APP_CONFIG for further analysis  **************" )
    exit(-1)
...
#app_id=1
df_group_config = sqlContext.sql("select * from %s where dq_app_id = %d and table_nm = '%s' " %(dq_group_config,dq_app_id,src_tbl)).limit(1).cache()
