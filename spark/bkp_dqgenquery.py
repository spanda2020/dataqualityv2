...
def dtlQuery(tbl,rowkey,col,nullChkRtn,dupChkRtn, lenChkRtn, lovChkRtn,dataTypeChkRtn,formatChkRtn,\
    ...
    fltrEnd = ' ) innerQuery where (nullChk = 1 or dupChk = 1 or lenChk = 1 or lovChk = 1 or dataTypeChk = 1 or \
                 formatChk = 1 or customChk = 1 or refChk = 1 )'
    if (refChkRtn[0:4]) != 'CASE' :
        s = 'SELECT '+ rowKeyValCal(tbl,rowkey)+','+tbl+'.'+col+',' + nullChkRtn+','+dupChkRtn+','+lenChkRtn+','+lovChkRtn+ ','+dataTypeChkRtn +','+\
