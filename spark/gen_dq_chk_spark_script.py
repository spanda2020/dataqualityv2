...
'dq_threshold_per','tot_rec','dq_run_status','dq_run_time','src_tbl','dq_batch_id']
...
app_id=1
df_group_config = sqlContext.sql("select * from %s where dq_app_id = %d " %(dq_group_config,app_id)).limit(1).cache()
...
for line in dfsql_chk_master.map(list).collect():
    ...
    dq_log_detl_stg = dq_log_detl_stg_import.withColumn("dq_run_dt",lit(dq_run_dt)).withColumn("dq_check_id",lit(dq_check_id))\
            ...
            .withColumn("dq_run_status",lit('STARTED')).withColumn("dq_run_time",lit(dq_run_start_time)).withColumn("dq_batch_id",lit(batch_id))
